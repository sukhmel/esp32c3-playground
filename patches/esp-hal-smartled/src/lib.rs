    ...
    Color: rgb::ComponentSlice<u8>,
{
        ...
        I: Into<Self::Color>,
    {
        ...
        for item in iterator {
            convert_to_pulses(rgb::ComponentSlice::as_slice(&item.into()), &mut seq_iter, self.pulses)?;
        }
        ...
        *seq_iter.next().ok_or(LedAdapterError::BufferSizeExceeded)? = PulseCode::end_marker();
        ...
        let transaction = match channel.transmit(self.rmt_buffer) {
            ...
            Err((e, chan)) => {
                ...
                return Err(LedAdapterError::from(e));
            }
        };
    ...
    Color: rgb::ComponentSlice<u8>,
{
    ...
    fn prepare_rmt_buffer<I: Into<Color>>(
        ...
        // the iterator provides more elements than the buffer can take.
        for item in iterator {
            Self::convert_to_pulses(rgb::ComponentSlice::as_slice(&item.into()), &mut seq_iter, self.pulses)?;
        }
        Ok(())
    }
